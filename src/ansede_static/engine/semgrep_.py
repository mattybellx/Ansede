...
def _build_ansede_finding(sr: dict, filepath: str) -> Finding:
    """Convert a semgrep result to an ansede Finding."""
    extra = sr.get("extra", {})
...
def run_semgrep_on_path(
    ...
    *,
    ...
    filepath = str(target)
    return [_build_ansede_finding(sr, filepath) for sr in raw_results]


def merge_semgrep_findings(
    ansede_findings: list[Finding],
    semgrep_findings: list[Finding],
) -> list[Finding]:
    """Merge semgrep findings into ansede findings with deduplication.

    Semgrep findings that overlap with ansede findings (same CWE + line)
    ...
    """
    ansede_keys: set[tuple[str, int]] = set()
    for f in ansede_findings:
        ansede_keys.add((f.cwe or "", f.line or 0))

    merged = list(ansede_findings)
    for sf in semgrep_findings:
        key = (sf.cwe or "", sf.line or 0)
        if key in ansede_keys:
            continue
